...
function stitch_img(pkmn1, pkmn2) {
    ...
    if (longest_run_len > longest_run_len_2) {
        ...
        for (let y = triangle; y < Math.ceil(img1.height/2) + triangle; y += 1) {
            let ycomp = y*img2.width * 4;
            for (let x = 0; x < img1.width; x += 1) {
                if (x+hdist >= guess_img.width || x+hdist < 0)
                    continue;

                let full = 4 * (guess_img.width) * (guess_img.height);
                let base = (x+hdist) * 4 + ycomp;
                if (base < full && base >= 0) {
                    guess_img.pixels[base] = img1.pixels[x * 4 + (y-triangle)*img1.width * 4];
                    guess_img.pixels[base + 1] = img1.pixels[x * 4 + (y-triangle)*img1.width * 4 + 1];
                    guess_img.pixels[base + 2] = img1.pixels[x * 4 + (y-triangle)*img1.width * 4 + 2];
                    guess_img.pixels[base + 3] = img1.pixels[x * 4 + (y-triangle)*img1.width * 4 + 3];
                }   
            }
        }
        ...
        for (let y = 0; y < Math.ceil(img2.height/2); y += 1) {
            let ycomp = (y+Math.ceil(img1.height/2))*img1.width * 4;
            for (let x = 0; x < img2.width; x += 1) {
                ...
                let base = (x+hdist) * 4 + ycomp;
                if (base < full && base >= 0) {
                    guess_img.pixels[base] = img2.pixels[x * 4 + (y+Math.ceil(img2.height/2))*img2.width * 4];
                    guess_img.pixels[base + 1] = img2.pixels[x * 4 + (y+Math.ceil(img2.height/2))*img2.width * 4 + 1];
                    guess_img.pixels[base + 2] = img2.pixels[x * 4 + (y+Math.ceil(img2.height/2))*img2.width * 4 + 2];
                    guess_img.pixels[base + 3] = img2.pixels[x * 4 + (y+Math.ceil(img2.height/2))*img2.width * 4 + 3];
                }
            }
        }
    }
